module Blogit
  ...
  class Configuration
    ...
    #  (default: nil)
    config_accessor(:disqus_shortname) do |shortname|
      using_disqus = include_comments == :disqus
      next nil if shortname.blank?
      blogit.warn "You've set config.disqus_shortname in your blogit config file but " \
        "config.include_comments is not set to :disqus" unless using_disqus
      shortname unless include_comments == :disqus
    end
